   ...
   public class Map
    {
       ...
       //make it so that when a map is constructed, it has an unpassable terrain value set and all other values
       //are passed to instantiate degrees of movement through terrain.  Default is 0 (not passable) and 1 (passable)
        ...
        public Map(float[,] map, int tWidth, int tHeight)
        {
            layout = map;
            tileWidth = tWidth;
            tileHeight = tHeight;
       ...
       /// </summary>
        ...
        public void ChangeTile(int row, int col, float tileType)
        {
            layout[col, row] = tileType;
        }
    }
    ...
    public class Pathfinder
    {
        ...
        public Map MapGrid
        {
            set
            {
                ...
                levelWidth = mapGrid.Width;

                levelHeight = mapGrid.Height;

                InitializeSearchNodes(mapGrid);
            }
        }
        ...
        public Pathfinder(float[,] map, int tileWidth, int tileHeight)
        {
            mapGrid = new Map(map, tileWidth, tileHeight);
            levelWidth = mapGrid.Width;
            levelHeight = mapGrid.Height;
            InitializeSearchNodes(mapGrid);
        }
        ...
        private List<Vector2> FindFinalPath(SearchNode startNode, SearchNode endNode)
        {
            ...
            {
                finalPath.Add(new Vector2(closedList[i].Position.X * mapGrid.TileWidth,
                                          closedList[i].Position.Y * mapGrid.TileHeight));
            }
        ...
        /// </remarks>
        /// <param name="p0">First Microsoft.Xna.Framework.Microsoft.Xna.Framework.Point</param>
        /// <param name="p1">Second Microsoft.Xna.Framework.Microsoft.Xna.Framework.Point</param>
        /// <param name="p2">Third Microsoft.Xna.Framework.Microsoft.Xna.Framework.Point</param>
        /// <param name="p3">Fourth Microsoft.Xna.Framework.Microsoft.Xna.Framework.Point</param>
        /// <param name="t">
        /// Normalised distance between second and third point 
        /// where the spline point will be calculated
        /// </param>
        /// <returns>
        /// Calculated Spline Microsoft.Xna.Framework.Microsoft.Xna.Framework.Point
        /// </returns>
        ...
        /// <param name="curve">A float value between 0.0f and 1.0f.  The bigger the number, the less points that will be added to the spline</param>
        /// </summary>
        public List<Vector2> SplineTransform(ref List<Vector2> input, float curve)
        {
            if (input.Count >= 4)
            {
                ...
                for (int i = 0; i < (input.Count - 3); i++)
                {
                    ...
                    //check to see if the points are in a straight line
                    if (!(points[0].X == points[1].X && points[0].X == points[2].X && points[0].X == points[3].X) &&
                        !(points[0].Y == points[1].Y && points[0].Y == points[2].Y && points[0].Y == points[3].Y) && curve > 0.0f && curve < 1.0f)
                    {
                        for (float t = 0.0f; t < 1.0f; t += curve)
                        {
                    ...
                    //since points are not in a straight line, no need to "spline" them
                    else
                        output.Add(input[i]);
