...
namespace Engine.ParticleSystem
{
    public class ParticleSystem : DrawableGameComponent
    {
        ...
        private List<Engine.Particle.Particle> particles;
        ...
        private Queue<Engine.Particle.Particle> freeParticles;

        // The settings used for this particle system
        private Engine.ParticlesSettings.ParticleSystemSettings settings;
        ...
        public ParticleSystem(Microsoft.Xna.Framework.Game game, string settingsAssetName)
            : this(game, settingsAssetName, 10)
        {
            /*if (particleSystem != null)
                throw new InvalidOperationException("Only one particle system of this component should be created.");
            particleSystem = this;*/
        ...
        public ParticleSystem(Microsoft.Xna.Framework.Game game, string settingsAssetName, int initialParticleCount)
            : base(game)
        {
            ...
            particles = new List<Engine.Particle.Particle>(initialParticleCount);
            freeParticles = new Queue<Engine.Particle.Particle>(initialParticleCount);
            for (int i = 0; i < initialParticleCount; i++)
            {
                particles.Add(new Engine.Particle.Particle());
        ...
        {
            // Load our settings
            settings = Game.Content.Load<Engine.ParticlesSettings.ParticleSystemSettings>(settingsAssetName);
        ...
        {
            float angle = Engine.ParticleHelpers.ParticleHelpers.RandomBetween(settings.MinDirectionAngle, settings.MaxDirectionAngle);
        ...
        public void AddParticles(Vector2 where, Vector2 velocity)
        {
            ...
            int numParticles =
                Engine.ParticleHelpers.ParticleHelpers.Random.Next(settings.MinNumParticles, settings.MaxNumParticles);
            ...
            for (int i = 0; i < numParticles; i++)
            {
                ...
                if (freeParticles.Count == 0)
                {
                    for (int j = 0; j < 10; j++)
                    {
                        Engine.Particle.Particle newParticle = new Engine.Particle.Particle();
                ...
                Engine.Particle.Particle p = freeParticles.Dequeue();
        ...
        private void InitializeParticle(Engine.Particle.Particle p, Vector2 where, Vector2 velocity)
        {
            ...
            float speed = Engine.ParticleHelpers.ParticleHelpers.RandomBetween(settings.MinInitialSpeed, settings.MaxInitialSpeed);
            velocity += direction * speed;

            // pick some random values for our particle
            float lifetime =
                Engine.ParticleHelpers.ParticleHelpers.RandomBetween(settings.MinLifetime, settings.MaxLifetime);
            float scale =
                Engine.ParticleHelpers.ParticleHelpers.RandomBetween(settings.MinSize, settings.MaxSize);
            float rotationSpeed = 
                Engine.ParticleHelpers.ParticleHelpers.RandomBetween(settings.MinRotationSpeed, settings.MaxRotationSpeed);
            ...
            switch (settings.AccelerationMode)
            {
                ...
                case Engine.ParticlesSettings.AccelerationMode.EndVelocity:
                    ...
                    break;
                case Engine.ParticlesSettings.AccelerationMode.Vector:
                    acceleration = new Vector2(
                        Engine.ParticleHelpers.ParticleHelpers.RandomBetween(settings.MinAccelerationVector.X, settings.MaxAccelerationVector.X),
                        Engine.ParticleHelpers.ParticleHelpers.RandomBetween(settings.MinAccelerationVector.Y, settings.MaxAccelerationVector.Y));
                    break;
                default:
                    break;
            }
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            foreach (Engine.Particle.Particle p in particles)
            {
